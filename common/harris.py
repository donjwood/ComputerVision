...
def get_harris_points(harrisim,min_dist=10,threshold=0.1): 
    ...
    filtered_coords = []
    for i in index:
        if allowed_locations[np.coords[i,0],np.coords[i,1]] == 1:
            filtered_coords.append(np.coords[i])
            allowed_locations[(np.coords[i,0]-min_dist):(np.coords[i,0]+min_dist), (np.coords[i,1]-min_dist):(np.coords[i,1]+min_dist)] = 0 
